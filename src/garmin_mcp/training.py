...
def register_tools(app):
    ...
    @app.tool()
    async def get_cycling_ftp(ctx: Context) -> str:
        ...
        """
        try:
            ftp_data = get_client(ctx).get_cycling_ftp()
    ...
    @app.tool()
    async def get_training_load_trend(ctx: Context, start_date: str, end_date: str) -> str:
    ...
    @app.tool()
    async def get_hrv_trend(ctx: Context, start_date: str, end_date: str) -> str:
        ...
        while current <= end:
            date_str = current.isoformat()
            try:
                data = get_client(ctx).get_hrv_data(date_str)
                if data:
    ...
    @app.tool()
    async def get_vo2max_trend(ctx: Context, start_date: str, end_date: str) -> str:
    ...
    async def get_respiration_trend(ctx: Context, start_date: str, end_date: str) -> str:
        ...
        while current <= end:
            date_str = current.isoformat()
            try:
                data = get_client(ctx).get_respiration_data(date_str)
                if data:
                    entry: Dict[str, Any] = {"date": date_str}
